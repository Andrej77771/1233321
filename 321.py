...
def init_db():
     try:
        conn = sqlite3.connect('user_librery.db')
        cursor = conn.cursor()
...
def show_book_details():
    try:
        ...
        if not selected_title:
            messagebox.showwarning("Bridinajums", "Ludzu, izveleties gramatu no saraksta.")
            return
...
root=Tk()
root.title("Bibliotekas Garumu Sistema")
...
Button(root, text="Rdit Informaciju", command=show_book_details).grid(row=2, column=0, padx=10, pady=10)
